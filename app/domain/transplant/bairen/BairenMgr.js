...
[
	{
		...
		maxCount: 50,// 最大人数
		entryCond: 100,// 进入条件
		upZhuangCond: 2000000,// 上庄条件
		singleBetLimit: 100000,// 单人押注上限
		rooms: [],//房间列表
		roomsVip:[]//vip房间
	}
];
...
exports.init = function (app) {
    ...
    stages.forEach(m=>{
        for(let i=0; i<games.minRoom; i++){
            ...
            m.rooms.push(new Room({nid:stages[0].id,id:roomCode, channel: app.channelService.createChannel('bairen'+roomCode)}));
        }
    });
};
...
exports.addGoldRecordBaiRen = function ({isVip,totalBet,totalWin,multiple,player,uid,moneyType},session) {
    if(!isVip){
